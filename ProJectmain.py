        ...
        def checkpwd() :
            ...
            if password=='123456' :
                ...
                def insertpro():
                    ...
                    frame = tk.Frame(windows, bg='#E6E6FA')
                    frame.place(relx = 0.5, rely=0.50, relwidth=0.57, relheight=0.35, anchor='n')
                    tk.Label(windows , text="กรุณาป้อนข้อมูลสินค้า").place(relx=0.5, rely=0.31, relwidth=0.25, anchor='n')

                    conn = sqlite3.connect("DatabaseProject.db")
                    c = conn.cursor()
                    row = c.execute('SELECT * FROM store ')
                    total = c.rowcount
                    #print('total Data Entries : '+str(total))
                    conn.commit()

                    tv = ttk.Treeview(windows, columns=(1,2,3,4), show='headings')
                    tv.place(relx=0.5, rely=0.38, relwidth=0.57,relheight=0.2, anchor='n')
                    ...
                    def data():
                            conn = sqlite3.connect("DatabaseProject.db")
                            c = conn.cursor()
                            c.execute('CREATE TABLE IF NOT EXISTS store (ID integer PRIMARY KEY AUTOINCREMENT , Products TEXT NOT NULL, Price TEXT NOT NULL, amount TEXT NOT NULL)')
                            conn.commit()
                            conn.close()
                    data()"""
                    ...
                    def savedata():
                        conn = sqlite3.connect("DatabaseProject.db")
                        c = conn.cursor()
                        c.execute('INSERT INTO store (Products, Price, amount) VALUES(?,?,?)',(box2.get(),box3.get(),box4.get()))
                        conn.commit()
                    ...
                    button2 = tk.Button(windows,text='ยกเลิก',bg="#CD5C5C",fg='#FFFFFF',command=creator_menu)
                ...
                def showpro():
                    ...
                    button1 = tk.Button(windows,text='กลับ',bg="#0066CC",fg='#FFFFFF',command=creator_menu)
                    button1.place(relx=0.35, rely=0.81, relwidth=0.125, relheight=0.08,anchor='n')

                    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
                    button2.place(relx=0.65, rely=0.81, relwidth=0.125, relheight=0.08,anchor='n')
                ...
                def updatepro():
                    ...
                    frame = tk.Frame(windows, bg='#f0f0f0')
                    frame.place(relx = 0.5, rely=0.52, relwidth=0.57, relheight=0.35, anchor='n')
                    ...
                    tv = ttk.Treeview(windows, columns=(1,2,3,4), show='headings')
                    tv.place(relx=0.5, rely=0.38, relwidth=0.55,relheight=0.15, anchor='n')
                ...
                def edit():
                    ...
                    conn = sqlite3.connect("DatabaseProject.db")
                    c = conn.cursor()
                    sql = 'UPDATE store SET Products = "{}", price = "{}", amount = "{}" WHERE ID = "{}"'.format(prod1,pric1,amou1,num1)
                    print(sql)
                    c.execute(sql)
                    conn.commit() 
                    conn.close()
                ...
                def deletepro():
                    ...
                    def deleteprodata():
                        ...
                        conn = sqlite3.connect("DatabaseProject.db")
                        c = conn.cursor()
                        c.execute('DELETE FROM store WHERE ID = ?  ' ,[data])
                        conn.commit() 
                        conn.close()
...
def sell(): 
    savetotal[0] = 0 #savetotal[0] คือ list ของจำนวนเงินที่รวมกัน
    Product_details.clear() #changmoney คือ list ที่เรียกข้อมูลจาก db มาใช้
...
def coffee():
    ...
    conn = sqlite3.connect("DatabaseProject.db")
    c = conn.cursor()
    sql = 'SELECT * FROM store WHERE Products = "{}"'.format(Name_Pro)
    c.execute(sql)
    ...
    for n in dataoutput :
        if Name_Pro == n[1] :
            ...
            Product_details.append(Amount)
            Product_details.append(Name_Pro)
            #print (changmoney)
            #print (Text)
...
def end():
    amount_del = int(Product_details[1])-1 #จำนวนสินค้าในคลัง - 1
    name_product = str(Product_details[2]) #ชื่อสินค้า
    print(amount_del)
    print(name_product)
    win_screen = tk.Frame(windows, bg= '#f0f0f0', bd=5)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
    ...
    price = float(savetotal[0]) #เงินที่กด
    get = float(Product_details[0]) #ราคาใน data
    if(price<get):
        ...
        sql = 'UPDATE store SET amount = "{}" WHERE Products = "{}"'.format(str(amount_del),name_product)
        print(sql)
        c.execute(sql)
        conn.commit() 
        conn.close()
...
def chang():
    ...
    Button(windows, text = "500 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 10,command=chang500).place(relx=0.43, rely=0.6, relwidth=0.1, relheight=0.1, anchor='n')
...
def chang20():
    ...
    Label(windows,text="เลือกเหรียญที่ต้องการ").place(relx=0.5, rely=0.27, relwidth=0.2, relheight=0.1, anchor='n')
    money = int(20)
    for t in moneytype:
        if money > t:
            ...
            kind.append(x)
            total = money % t
    Label(windows,text="อัตราการแลกเงิน แบงก์ 20 บาท\n"+kind[0]+"\n"+kind[1]+"\n"+kind[2]+"\n"+kind[3]
        ,bg="#D2691E",fg='#FFFFFF').place(relx=0.35, rely=0.4, relwidth=0.28, relheight=0.35,anchor='n')

    Button(windows, text = "10 บาท",fg="#000000" ,bg="#98FB98", width = 5,command=change_change20).place(relx=0.60, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "5 บาท",fg="#000000" ,bg="#66CDAA", width = 5,command=change1_change20).place(relx=0.73, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "2 บาท",fg="#000000" ,bg="#FFA07A", width = 5,command=change2_change20).place(relx=0.60, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "1 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 5,command=change3_change20).place(relx=0.73, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')


    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='แลกอีกครั้ง',bg="#CC0066",fg='#FFFFFF',command=chang)
...
def chang50():
    ...
    for t in moneytype:
        if money > t:
            ...
            kind.append(x)
            total = money % t
    Label(windows,text="อัตราการแลกเงิน แบงก์ 50 บาท\n"+kind[0]+"\n"+kind[1]+"\n"+kind[2]+"\n"+kind[3]
        ,bg="#D2691E",fg='#FFFFFF').place(relx=0.35, rely=0.4, relwidth=0.28, relheight=0.35,anchor='n')

    Button(windows, text = "10 บาท",fg="#000000" ,bg="#98FB98", width = 5,command=change_change50).place(relx=0.60, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "5 บาท",fg="#000000" ,bg="#66CDAA", width = 5,command=change1_change50).place(relx=0.73, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "2 บาท",fg="#000000" ,bg="#FFA07A", width = 5,command=change2_change50).place(relx=0.60, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "1 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 5,command=change3_change50).place(relx=0.73, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
...
def chang100():
    ...
    for t in moneytype:
        if money > t:
            ...
            kind.append(x)
            total = money % t
    Label(windows,text="อัตราการแลกเงิน แบงก์ 100 บาท\n"+kind[0]+"\n"+kind[1]+"\n"+kind[2]+"\n"+kind[3]
        ,bg="#D2691E",fg='#FFFFFF').place(relx=0.35, rely=0.4, relwidth=0.28, relheight=0.35,anchor='n')

    Button(windows, text = "10 บาท",fg="#000000" ,bg="#98FB98", width = 5,command=change_change100).place(relx=0.60, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "5 บาท",fg="#000000" ,bg="#66CDAA", width = 5,command=change1_change100).place(relx=0.73, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "2 บาท",fg="#000000" ,bg="#FFA07A", width = 5,command=change2_change100).place(relx=0.60, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "1 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 5,command=change3_change100).place(relx=0.73, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
...
def chang500():
    ...
    for t in moneytype:
        if money > t:
            ...
            kind.append(x)
            total = money % t
    Label(windows,text="อัตราการแลกเงิน แบงก์ 500 บาท\n"+kind[0]+"\n"+kind[1]+"\n"+kind[2]+"\n"+kind[3]
        ,bg="#D2691E",fg='#FFFFFF').place(relx=0.35, rely=0.4, relwidth=0.28, relheight=0.35,anchor='n')

    Button(windows, text = "10 บาท",fg="#000000" ,bg="#98FB98", width = 5,command=change_change500).place(relx=0.60, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "5 บาท",fg="#000000" ,bg="#66CDAA", width = 5,command=change1_change500).place(relx=0.73, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "2 บาท",fg="#000000" ,bg="#FFA07A", width = 5,command=change2_change500).place(relx=0.60, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "1 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 5,command=change3_change500).place(relx=0.73, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
...
def chang1000():
    ...
    for t in moneytype:
        if money > t:
            ...
            kind.append(x)
            total = money % t
    Label(windows,text="อัตราการแลกเงิน แบงก์ 1000 บาท\n"+kind[0]+"\n"+kind[1]+"\n"+kind[2]+"\n"+kind[3]
        ,bg="#D2691E",fg='#FFFFFF').place(relx=0.35, rely=0.4, relwidth=0.28, relheight=0.35,anchor='n')

    Button(windows, text = "10 บาท",fg="#000000" ,bg="#98FB98", width = 5,command=change_change1000).place(relx=0.60, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "5 บาท",fg="#000000" ,bg="#66CDAA", width = 5,command=change1_change1000).place(relx=0.73, rely=0.40, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "2 บาท",fg="#000000" ,bg="#FFA07A", width = 5,command=change2_change1000).place(relx=0.60, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
    Button(windows, text = "1 บาท",fg="#FFFFFF" ,bg="#9932CC", width = 5,command=change3_change1000).place(relx=0.73, rely=0.55, relwidth=0.1, relheight=0.08, anchor='n')
...
def change_change20():
    ...
    Label(windows,text="แลกเหรียญสำเร็จ").place(relx=0.5, rely=0.27, relwidth=0.2, relheight=0.1, anchor='n')
    money = int(20)
    for t in moneytype:
        if money > t:
            x = ("{:<5}บาท\tจำนวน\t{:<10}".format(t , int(money / t)))
    ...
    Label(windows,text="อัตราการแลกเงิน\n"+kind[0]
        ,bg="#1E90FF",fg='#FFFFFF').place(relx=0.5, rely=0.4, relwidth=0.3, relheight=0.28,anchor='n')

    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='กลับ',bg="#6A5ACD",fg='#FFFFFF',command=chang20)
    button3.place(relx=0.62, rely=0.8, relwidth=0.15, relheight=0.08,anchor='n')

    creator = tk.Frame(windows, bg='#006699', bd=6) 
    creator.place(relx=0.5, rely=0.9, relwidth=0.57, relheight=0.06, anchor='n') #กรอบล่าง

def change1_change20():
    kind=[]
    win_screen = tk.Frame(windows)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
...
def change_change50():
    ...
    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='กลับ',bg="#6A5ACD",fg='#FFFFFF',command=chang50)
    button3.place(relx=0.62, rely=0.8, relwidth=0.15, relheight=0.08,anchor='n')

    creator = tk.Frame(windows, bg='#006699', bd=6) 
    creator.place(relx=0.5, rely=0.9, relwidth=0.57, relheight=0.06, anchor='n') #กรอบล่าง

def change1_change50():
    kind=[]
    win_screen = tk.Frame(windows)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
...
def change_change100():
    ...
    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='กลับ',bg="#6A5ACD",fg='#FFFFFF',command=chang100)
    button3.place(relx=0.62, rely=0.8, relwidth=0.15, relheight=0.08,anchor='n')

    creator = tk.Frame(windows, bg='#006699', bd=6) 
    creator.place(relx=0.5, rely=0.9, relwidth=0.57, relheight=0.06, anchor='n') #กรอบล่าง
def change1_change100():
    kind=[]
    win_screen = tk.Frame(windows)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
...
def change_change500():
    ...
    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='กลับ',bg="#6A5ACD",fg='#FFFFFF',command=chang500)
    button3.place(relx=0.62, rely=0.8, relwidth=0.15, relheight=0.08,anchor='n')

    creator = tk.Frame(windows, bg='#006699', bd=6) 
    creator.place(relx=0.5, rely=0.9, relwidth=0.57, relheight=0.06, anchor='n') #กรอบล่าง
def change1_change500():
    kind=[]
    win_screen = tk.Frame(windows)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
...
def change_change1000():
    ...
    button2 = tk.Button(windows,text='กลับหน้าหลัก',bg="#2E8B57",fg='#FFFFFF',command=home)
    button2.place(relx=0.40, rely=0.8, relwidth=0.19, relheight=0.08,anchor='n')
    button3 = tk.Button(windows,text='กลับ',bg="#6A5ACD",fg='#FFFFFF',command=chang1000)
    button3.place(relx=0.62, rely=0.8, relwidth=0.15, relheight=0.08,anchor='n')

    creator = tk.Frame(windows, bg='#006699', bd=6) 
    creator.place(relx=0.5, rely=0.9, relwidth=0.57, relheight=0.06, anchor='n') #กรอบล่าง
def change1_change1000():
    kind=[]
    win_screen = tk.Frame(windows)
    win_screen.place(relx=0.5, rely=0.27, relwidth=1.2, relheight=0.8, anchor='n')
